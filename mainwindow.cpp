...
{
    for(int count = 0; count < getMemType(member.getTotal()),count++)
    {
    this->bulkClub.getMemType(executive.getRebateAmount);
    }
}
...
void MainWindow::on_pushButton_14_clicked()
{
   for (int count = 0; count < getMemType(member.getTotal), count++)
    {
    this->bulkClub.getMemType(executive.getExprDate);
    }
}
